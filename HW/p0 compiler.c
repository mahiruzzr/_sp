...
// =========================================================
typedef enum {
    TK_FUNC, TK_RETURN, TK_IF, TK_WHILE,   // <-- 新增 TK_WHILE
    TK_ID, TK_NUM,
...
void next_token() {
    ...
    if (isdigit(*src)) {
        ...
        else if (strcmp(cur_token.text, "while")  == 0) cur_token.type = TK_WHILE; // <-- 新增
...
void statement() {
    ...
    // -------------------------------------------------------
        ...
        int loop_start = quad_count;            // 記錄條件判斷入口（回跳目標）
        char cond[32]; expression(cond);
        next_token(); next_token();             // skip ')' '{'

        int jmp_out_idx = quad_count;
        emit("JMP_F", cond, "-", "?");         // 條件為假跳出迴圈
...
void vm() {
    ...
    while (pc < quad_count) {
        Quad q = quads[pc];
        if      (strcmp(q.op, "FUNC_BEG") == 0) { while (strcmp(quads[pc].op, "FUNC_END") != 0) pc++; }
        ...
        else if (strcmp(q.op, "JMP_F")  == 0) { if (get_var(q.arg1) == 0) { pc = atoi(q.result) - 1; } }
        else if (strcmp(q.op, "JMP")    == 0) { pc = atoi(q.result) - 1; }  // <-- 新增無條件跳轉
